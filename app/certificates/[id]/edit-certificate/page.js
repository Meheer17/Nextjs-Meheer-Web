...
export default function EditProject({params}) {
    ...
    const Data = () => {
        ...
        const [form, setForm] = useState({title: '', description: '' ,ranid: '', link: '', image: ''})
        ...
        async function deb(){
            if(form.title == "" && found==false){
                ...
                const res = await fetch(`https://meheer.vercel.app/api/certificates?id=${search}`);
        ...
        const createProject = async () => {
            try {
                const res = await fetch(`https://meheer.vercel.app/api/certificates?id=${form.ranid}`, {
                    method: 'PUT',
        ...
        const dele = () => {
            try {
                const res = fetch(`https://meheer.vercel.app/api/certificates/delete/${fileId}`, {
                    method: 'GET',
                    headers:{
                ...
                
               {
                isSubmitting ? <div className="mx-auto text-center w-10 h-10 pt-10 "><Loader/></div> : (
                    ...
                    <div className="p-10 drop-shadow-xl ">
                        <form className="mx-auto max-w-5xl bg-slate-300 p-5 border-2 border-slate-900 rounded-md" onSubmit={handleSubmit}>
                            <Image height={500} width={1000} className='mb-2' src={form.image} />
